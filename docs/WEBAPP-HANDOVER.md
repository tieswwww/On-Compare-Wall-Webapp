...
> Plain language, explains the *why*, not just the *what*.
>
...
- **Bottom quadrant (per side):** a two-stage reveal — first a **colour drape** in the
  ...
  **Energy/experience** header, three **bar graphs** (Cushioning / Responsiveness /
  ...
  Distance**).
- **One shoe only:** the *opposite* empty half shows that shoe's full-height **"Key
...
other kiosk/tab. So `ensureUser` only *creates*, never *updates*.
...
- The app keys the catalog by **whatever EAN arrives** — so the open question is purely
  *which* EAN population lives in `shoes` (see §7), not the wire format.
...
1. **Catalog population — the big one (in flux).** The app reads its **own** flat
   `shoes` table, not TSC's `compare_wall`. *How that table gets filled* is being
...
- **Broadcast vs DB write:** the app reacts to a Realtime *broadcast* (fast), and
  *separately* persists to `shoe_slots`/`shoe_events` (audit). Broadcast comes first.
...
*Counterpart to the hardware/data handovers in the other repo. The events seam (§6) is
the most settled; the catalog-population model (§7.1) is the main thing still being
decided. If you're an AI reading this: start at §4 (data) and §7 (open questions).*
